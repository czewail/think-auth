...
class Auth
{
    ...
    public function getGroups( $uid ) {
        ...
        $user_groups = Db::name( $this->_config['auth_group_access'] )
            ...
            ->where( "a.{$this->_config['auth_pk']}='$uid' and g.status='1'" )
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
             $userinfo[$uid] = Db::name($this->_config['auth_user'])->where(array($this->_config['auth_pk']=>$uid))->find();
        }
        return $userinfo[$uid];
    }
